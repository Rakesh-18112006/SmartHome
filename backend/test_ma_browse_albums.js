...
ws.on('message', (data) => {
  ...
  if (msg.result?.authenticated) {
    ws.send(JSON.stringify({
      ...
      args: { path: "builtin://albums" }
    }));
    ws.send(JSON.stringify({
      message_id: String(msgId++),
      command: "music/browse",
      args: { path: "builtin://folder/albums" }
    }));
  }
  
  if (msg.message_id === "2") {
    console.log(`Msg ${msg.message_id} (builtin://albums):`, JSON.stringify(msg, null, 2).slice(0, 1000));
  }
  if (msg.message_id === "3") {
    console.log(`Msg ${msg.message_id} (builtin://folder/albums):`, JSON.stringify(msg, null, 2).slice(0, 1000));
    process.exit(0);
  }
});
